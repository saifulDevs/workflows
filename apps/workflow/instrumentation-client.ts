/**
 * workflow Studio Telemetry - Client-side Instrumentation
 *
...
if (typeof window !== 'undefined') {
  const TELEMETRY_STATUS_KEY = 'workflows-telemetry-status'
  ...
  ;(window as any).__workflow_TELEMETRY_ENABLED = telemetryEnabled
  ;(window as any).__workflow_TRACK_EVENT = (eventName: string, properties?: any) => {
  ...
  if (telemetryEnabled) {
    performance.mark('workflow-studio-init')

    let telemetryConfig
    try {
      telemetryConfig = (window as any).__workflow_STUDIO_TELEMETRY_CONFIG || {
    ...
    window.addEventListener('load', () => {
      performance.mark('workflow-studio-loaded')
      performance.measure('page-load', 'workflow-studio-init', 'workflow-studio-loaded')
