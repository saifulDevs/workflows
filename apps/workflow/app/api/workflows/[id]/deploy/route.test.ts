...
describe('Workflow Deployment API Route', () => {
  beforeEach(() => {
    ...
    vi.doMock('@/lib/utils', () => ({
      generateApiKey: vi.fn().mockReturnValue('workflow_testkeygenerated12345'),
    }))
  ...
  it('should create new API key when deploying workflow for user with no API key', async () => {
    ...
    expect(data).toHaveProperty('apiKey', 'workflow_testkeygenerated12345')
  ...
  it('should use existing API key when deploying workflow', async () => {
      ...
      db: {
        select: vi
          ...
          .mockReturnValueOnce({
            from: vi.fn().mockReturnValue({
              where: vi.fn().mockReturnValue({
                limit: vi.fn().mockResolvedValue([{ key: 'workflow_existingtestapikey12345' }]), // Existing API key
              }),
            }),
          }),
        insert: vi.fn().mockImplementation(() => ({
    ...
    expect(data).toHaveProperty('apiKey', 'workflow_existingtestapikey12345')
    expect(data).toHaveProperty('isDeployed', true)
  })
