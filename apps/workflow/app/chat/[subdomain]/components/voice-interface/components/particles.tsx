...
class workflowpleBloomComposer {
...
export function ParticlesVisualization({
  ...
  const bloomComposerRef = useRef<workflowpleBloomComposer | null>(null)
  ...
  useEffect(() => {
    ...
    const bloomComposer = new workflowpleBloomComposer(renderer, scene, camera)
    ...
    const calculateAudioIntensity = (elapsedTime: number, avgLevel: number) => {
      ...
      if (isMuted) {
        ...
        // Strong animation when AI is speaking - use workflowulated levels + enhancement
