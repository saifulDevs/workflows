...
export class DocsChunker {
  ...
  constructor(options: DocsChunkerOptions = {}) {
    ...
    this.baseUrl =
      options.baseUrl ?? (isDev ? 'http://localhost:3001' : 'https://docs.workflows.ai')
  }
  ...
  async chunkMdxFile(filePath: string, basePath: string): Promise<DocChunk[]> {
    ...
    for (let i = 0; i < textChunks.length; i++) {
      ...
      const chunk: DocChunk = {
        text: chunkText,
        tokenCount: Math.ceil(chunkText.length / 4), // workflowple token estimation
  ...
  private parseFrontmatter(content: string): { data: Frontmatter; content: string } {
    ...
    // workflowple YAML parsing for title and description
