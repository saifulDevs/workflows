...
const NETWORK_NAME = 'workflows-network'
const DB_CONTAINER = 'workflows-db'
const MIGRATIONS_CONTAINER = 'workflows-migrations'
const REALTIME_CONTAINER = 'workflows-realtime'
const APP_CONTAINER = 'workflows-app'
const DEFAULT_PORT = '3000'

const program = new Command()

program.name('workflows').description('Run workflow Studio using Docker').version('0.1.0')

program
  .option('-p, --port <port>', 'Port to run workflow Studio on', DEFAULT_PORT)
...
async function main() {
  ...
  console.log(chalk.blue('🚀 Starting workflow Studio...'))
  ...
  if (options.pull) {
    await pullImage('ghcr.io/workflowsai/workflows:latest')
    await pullImage('ghcr.io/workflowsai/migrations:latest')
    await pullImage('ghcr.io/workflowsai/realtime:latest')
  ...
  const dataDir = join(homedir(), '.workflows', 'data')
  ...
  const migrationsSuccess = await runCommand([
    ...
    NETWORK_NAME,
    '-e',
    `DATABASE_URL=postgresql://postgres:postgres@${DB_CONTAINER}:5432/workflows`,
    'ghcr.io/workflowsai/migrations:latest',
    'bun',
    'run',
    'db:migrate',
  ])
  ...
  const realtimeSuccess = await runCommand([
    ...
    'ghcr.io/workflowsai/realtime:latest',
  ])
  ...
  const appSuccess = await runCommand([
    ...
    'ghcr.io/workflowsai/workflows:latest',
  ])

  if (!appSuccess) {
    console.error(chalk.red('❌ Failed to start workflow Studio'))
    process.exit(1)
  }

  console.log(
    chalk.green(`✅ workflow Studio is now running at ${chalk.bold(`http://localhost:${port}`)}`)
  )
  console.log(
    chalk.yellow(
      `🛑 To stop all containers, run: ${chalk.bold('docker stop workflows-app workflows-db workflows-realtime')}`
    )
  )
  ...
  rl.on('SIGINT', async () => {
    ...
    console.log(chalk.green('✅ workflow Studio has been stopped'))
    process.exit(0)
  })
}
