...
export async function GET(req: NextRequest) {
  ...
  if (!token) {
    ...
    return NextResponse.redirect(
      new URL(
        ...
        env.NEXT_PUBLIC_APP_URL || 'https://workflows.ai'
      )
    )
  }
  ...
  if (!session?.user?.id) {
    ...
    return NextResponse.redirect(
      new URL(`/invite/${token}?token=${token}`, env.NEXT_PUBLIC_APP_URL || 'https://workflows.ai')
    )
  }
  ...
  try {
    ...
    const isValidMatch = isExactMatch || (isSameDomain && isworkflowilarUsername)
