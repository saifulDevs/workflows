...
describe('Nested Routing Fix - Router → Condition → Target', () => {
  ...
  beforeEach(() => {
    // Create a workflow workflowilar to the screenshot: Router → Condition → Function/Parallel
    workflow = {
      version: '2.0',
      blocks: [
        {
  ...
  it('should prevent parallel blocks from executing when not selected by nested routing', () => {
    // This test workflowulates the exact scenario from the bug report
    ...
    // Step 3: workflowulate what the executor's getNextExecutionLayer would do
