...
vi.mock('../env', () => ({
  env: {
    ...
    NEXT_PUBLIC_APP_URL: 'https://test.workflows.ai',
  },
}))

vi.mock('../urls/utils', () => ({
  getEmailDomain: vi.fn().mockReturnValue('workflows.ai'),
}))
  ...
  describe('sendEmail', () => {
    it('should send a transactional email successfully', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        from: 'workflow Studio <noreply@workflows.ai>',
    ...
    it('should send a marketing email with unsubscribe headers', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith({
        ...
        headers: {
          'List-Unsubscribe':
            '<https://test.workflows.ai/unsubscribe?token=mock-token-123&email=test%40example.com>',
    ...
    it.concurrent('should use custom from address when provided', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith(
        expect.objectContaining({
          from: 'workflow Studio <custom@example.com>',
        })
      )
    })
