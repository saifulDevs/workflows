...
The Parallel block is a container block in workflow Studio that allows you to execute multiple instances of blocks concurrently.
...
<Callout type="info">
  Parallel blocks are container nodes that execute their contents multiple times workflowultaneously, unlike loops which execute sequentially.
</Callout>
...
<Steps>
  ...
  <Step>
    <strong>Speed up execution</strong>: Run independent operations workflowultaneously
  </Step>
  <Step>
...
<Tabs items={['Count-based', 'Collection-based']}>
  <Tab>
    ...
    - Instance 1 ┐
    - Instance 2 ├─ All execute workflowultaneously
    - Instance 3 │
    - Instance 4 │
    - Instance 5 ┘
    ```
  ...
  <Tab>
    Distribute a collection across parallel instances. Each instance processes one item from the collection workflowultaneously.
    
    ```
    Example: Process ["task1", "task2", "task3"] in parallel
    - Instance 1: Process "task1" ┐
    - Instance 2: Process "task2" ├─ All execute workflowultaneously
...
<div className="mb-4 rounded-md border p-4">
  <h4 className="font-medium">Scenario: Process multiple API calls workflowultaneously</h4>
