# Contributing to workflow Studio

Thank you for your interest in contributing to workflow Studio! Our goal is to provide developers with a powerful, user-friendly platform for building, testing, and optimizing agentic workflows. We welcome contributions in all forms—from bug fixes and design improvements to brand-new features.

> **Project Overview:**  
> workflow Studio is a monorepo using Turborepo, containing the main application (`apps/workflow/`), documentation (`apps/docs/`), and shared packages (`packages/`). The main application is built with Next.js (app router), ReactFlow, Zustand, Shadcn, and Tailwind CSS. Please ensure your contributions follow our best practices for clarity, maintainability, and consistency.
...
We strive to keep our workflow as workflowple as possible. To contribute:
...
### Option 1: Using NPM Package (workflowplest)

The easiest way to run workflow Studio locally is using our NPM package:
...
3. **Start Developing:**
   ...
   - Run `bun run dev:full` in the terminal or use the `workflow-start` alias
...
3. **Templates Location:**
   - Email templates are located in `workflow/app/emails/`
...
workflow Studio is built in a modular fashion where blocks and tools extend the platform's functionality. To maintain consistency and quality, please follow the guidelines below when adding a new block or tool.

### Where to Add Your Code

- **Blocks:** Create your new block file under the `/apps/workflow/blocks/blocks` directory. The name of the file should match the provider name (e.g., `pinecone.ts`).
- **Tools:** Create a new directory under `/apps/workflow/tools` with the same name as the provider (e.g., `/apps/workflow/tools/pinecone`).

In addition, you will need to update the registries:

- **Block Registry:** Update the blocks index (`/apps/workflow/blocks/index.ts`) to include your new block.
- **Tool Registry:** Update the tools registry (`/apps/workflow/tools/index.ts`) to add your new tool.

### How to Create a New Block

1. **Create a New File:**  
   Create a file for your block named after the provider (e.g., `pinecone.ts`) in the `/apps/workflow/blocks/blocks` directory.

2. **Create a New Icon:**
   Create a new icon for your block in the `/apps/workflow/components/icons.tsx` file. The icon should follow the same naming convention as the block (e.g., `PineconeIcon`).

3. **Define the Block Configuration:**  
   Your block should export a constant of type `BlockConfig`. For example:

   ```typescript:/apps/workflow/blocks/blocks/pinecone.ts
...
4. **Register Your Block:**  
   Add your block to the blocks registry (`/apps/workflow/blocks/registry.ts`):

   ```typescript:/apps/workflow/blocks/registry.ts
...
1. **Create a New Directory:**  
   Create a directory under `/apps/workflow/tools` with the same name as the provider (e.g., `/apps/workflow/tools/pinecone`).
...
4. **Create an Index File:**  
   ...
   ```typescript:/apps/workflow/tools/pinecone/index.ts
...
5. **Define the Tool Configuration:**  
   ...
   ```typescript:/apps/workflow/tools/pinecone/fetch.ts
...
6. **Register Your Tool:**  
   Update the tools registry in `/apps/workflow/tools/index.ts` to include your new tool:

   ```typescript:/apps/workflow/tools/index.ts
...
workflow Studio implements a sophisticated parameter visibility system that controls how parameters are exposed to users and LLMs in agent workflows. Each parameter can have one of four visibility levels:
...
In addition, by submitting your contributions, you grant workflow Studio, Inc. ("The Licensor") a perpetual, irrevocable, worldwide, royalty-free, sublicensable right and license to:
...
Thank you for taking the time to contribute to workflow Studio. We truly appreciate your efforts and look forward to collaborating with you!
