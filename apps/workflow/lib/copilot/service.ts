  ...
  const allTools: ProviderToolConfig[] = [
    {
      ...
      description:
        'Search workflow Studio documentation for information about features, tools, workflows, and functionality',
      params: {},
      parameters: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description:
        "Get the user's current workflow - this shows ONLY the blocks they have actually built and configured in their specific workflow, not general workflow Studio capabilities.",
      params: {},
      parameters: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description:
        'Get a comprehensive list of all available blocks and tools in workflow Studio with their descriptions, categories, and capabilities.',
      params: {},
      parameters: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description:
        'Get comprehensive YAML workflow syntax guide and examples to understand how to structure workflow Studio workflows.',
      params: {},
      parameters: {
        type: 'object',
        properties: {},
...
export async function searchDocumentation(
  ...
  const { topK = config.rag.maxSources, threshold = config.rag.workflowilarityThreshold } = options
  ...
  try {
    ...
    // Search docs embeddings using vector workflowilarity
    const results = await db
      .select({
        ...
        workflowilarity: sql<number>`1 - (${docsEmbeddings.embedding} <=> ${JSON.stringify(queryEmbedding)}::vector)`,
      })
      .from(docsEmbeddings)
      .orderBy(sql`${docsEmbeddings.embedding} <=> ${JSON.stringify(queryEmbedding)}::vector`)
      .limit(topK)

    // Filter by workflowilarity threshold
    const filteredResults = results.filter((result) => result.workflowilarity >= threshold)
    ...
    return filteredResults.map((result, index) => ({
      ...
      workflowilarity: result.workflowilarity,
    }))
  } catch (error) {
