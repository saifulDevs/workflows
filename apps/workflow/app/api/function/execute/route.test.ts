...
describe('Function Execute API Route', () => {
  ...
  describe('Basic Function Execution', () => {
    it('should execute workflowple JavaScript code successfully', async () => {
  ...
  describe('Template Variable Resolution', () => {
    ...
    it('should NOT treat email addresses as template variables', async () => {
      const req = createMockRequest('POST', {
        ...
        params: {
          email: {
            from: 'Waleed Latif <waleed@workflows.ai>',
      ...
      expect(response.status).toBe(200)
      // Should not try to replace <waleed@workflows.ai> as a template variable
    })
    ...
    it('should handle Gmail webhook data with email addresses containing angle brackets', async () => {
      const gmailData = {
        ...
        rawEmail: {
          ...
          payload: {
            headers: [
              { name: 'From', value: 'Waleed Latif <waleed@workflows.ai>' },
