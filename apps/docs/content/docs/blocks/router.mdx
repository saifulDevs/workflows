...
The Router block uses AI to intelligently decide which path your workflow should take next. Unlike Condition blocks that use workflowple rules, Router blocks can understand context and make smart routing decisions based on content analysis.
...
<Steps>
  ...
  <Step>
    <strong>Context-aware decisions</strong>: Make smart routing choices beyond workflowple rules
  </Step>
  <Step>
...
<Accordions>
  ...
  <Accordion title="When to Use Condition">
    - workflowple, rule-based decisions
...
```javascript
// Fast routing for workflowple cases
Model: GPT-4o-mini
Criteria: workflowple, common routing patterns
