...
description: Understanding the fundamental execution flow in workflow Studio
---
...
When you run a workflow in workflow Studio, the execution engine follows a systematic process to ensure blocks are executed in the correct order with proper data flow.
...
workflow Studio offers multiple ways to trigger workflow execution:

### Manual Execution

Run workflows on-demand through the workflow Studio interface by clicking the "Run" button. This is perfect for:
