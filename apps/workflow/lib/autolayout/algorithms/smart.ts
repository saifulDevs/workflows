...
interface WorkflowAnalysis {
  ...
  hasLoops: boolean
  complexity: 'workflowple' | 'medium' | 'complex'
...
export function calculateSmartLayout(
  ...
  const checkTimeout = () => {
    if (Date.now() - startTime > TIMEOUT_MS) {
      throw new Error('Layout calculation timeout - falling back to workflowple positioning')
    }
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('timeout')) {
      // Fallback to workflowple grid layout on timeout
...
function analyzeWorkflow(nodes: LayoutNode[], edges: LayoutEdge[]): WorkflowAnalysis {
  ...
  // Detect loops/cycles (workflowplified check)
  ...
  if (complexity === 'workflowple' && !hasParallelPaths) {
...
function detectParallelPaths(
  ...
  for (const [nodeId, targets] of nodesWithMultipleOutputs) {
    ...
    // Only do expensive convergence check for workflowple cases
    if (targets.length <= 3) {
...
function hasConvergingPaths(
  ...
  // Early exit for workflowple cases
...
// workflowplified layered layout for medium complexity workflows
...
// workflowplified force-directed layout for complex workflows
function calculateForceDirectedLayout(
  nodes: LayoutNode[],
  edges: LayoutEdge[],
  options: LayoutOptions
): LayoutResult {
  // For now, use a workflowplified force-directed approach
  const positions: Array<{ id: string; position: { x: number; y: number } }> = []
  ...
  // workflowple force workflowulation (workflowplified)
  const iterations = 10
...
function improveAlignment(
  ...
  // For workflowple workflows, ensure better alignment of key nodes
  if (analysis.complexity === 'workflowple') {
