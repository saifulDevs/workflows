...
export class workflowsClient {
  ...
  constructor(config: workflowsConfig) {
    ...
    this.baseUrl = (config.baseUrl || 'https://workflows.ai').replace(/\/+$/, '')
  }
  ...
  async executeWorkflow(
    ...
    try {
      ...
      if (error.message === 'TIMEOUT') {
        throw new workflowsError(`Workflow execution timed out after ${timeout}ms`, 'TIMEOUT')
      }

      throw new workflowsError(error?.message || 'Failed to execute workflow', 'EXECUTION_ERROR')
    }
  }
  ...
  async getWorkflowStatus(workflowId: string): Promise<WorkflowStatus> {
    ...
    try {
      ...
      throw new workflowsError(error?.message || 'Failed to get workflow status', 'STATUS_ERROR')
    }
  }
...
export { workflowsClient as default }
