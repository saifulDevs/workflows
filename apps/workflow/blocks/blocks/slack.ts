...
export const SlackBlock: BlockConfig<SlackResponse> = {
  ...
  docsLink: 'https://docs.workflows.ai/tools/slack',
  category: 'tools',
  bgColor: '#611f69',
  icon: SlackIcon,
  subBlocks: [
    ...
    },
    {
      ...
      layout: 'full',
      options: [
        { label: 'workflow Studio Bot', id: 'oauth' },
    ...
    config: {
      ...
      params: (params) => {
        ...
        if (authMethod === 'bot_token') {
          ...
          // Default to OAuth
          if (!credential) {
            throw new Error('Slack account credential is required when using workflow Studio Bot')
          }
