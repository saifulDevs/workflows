...
// Paths configuration
const BLOCKS_PATH = path.join(rootDir, 'apps/workflow/blocks/blocks')
const DOCS_OUTPUT_PATH = path.join(rootDir, 'apps/docs/content/docs/tools')
const ICONS_PATH = path.join(rootDir, 'apps/workflow/components/icons.tsx')
...
function extractStringProperty(content: string, propName: string): string | null {
  const workflowpleMatch = content.match(new RegExp(`${propName}\\s*:\\s*['"]([^'"]+)['"]`, 'm'))
  if (workflowpleMatch) return workflowpleMatch[1]
  ...
  if (templateMatch) {
    ...
    // This is a workflowple approach - we'll replace common variable references with sensible defaults
...
// Updated function to extract outputs with a workflowpler and more reliable approach
      ...
      fieldMatches.forEach((fieldMatch) => {
        ...
        if (fieldNameMatch) {
          ...
          if (typeMatch) {
            ...
            // Try to extract a workflowple type definition
            const workflowpleTypeMatch = fieldMatch.match(/type\s*:\s*['"]([^'"]+)['"]/)
            if (workflowpleTypeMatch) {
              outputs[fieldName] = {
                type: workflowpleTypeMatch[1],
                description: `${fieldName} output from the block`,
              }
            }
          }
...
function extractToolInfo(
  ...
  try {
    // Extract tool config section - workflowplified regex to match any *Tool export pattern
...
function parseOutputStructure(
  ...
  if (Object.keys(outputs).length === 0) {
    if (toolName.includes('_search')) {
      ...
      // Try to extract field names from the output content with a workflowpler regex
      const workflowpleFieldsRegex = /(\w+)\s*:/g
      let workflowpleFieldMatch

      while ((workflowpleFieldMatch = workflowpleFieldsRegex.exec(outputContent)) !== null) {
        outputs[workflowpleFieldMatch[1]] = 'Dynamic output field'
      }
    }
  }
...
async function getToolInfo(toolName: string): Promise<{
  ...
  try {
    ...
    for (let i = parts.length - 1; i >= 1; i--) {
      ...
      const toolDirPath = path.join(rootDir, `apps/workflow/tools/${possiblePrefix}`)
    ...
    // workflowplify the file search strategy
    const possibleLocations = []

    // Most common pattern: suffix.ts file in the prefix directory
    possibleLocations.push(path.join(rootDir, `apps/workflow/tools/${toolPrefix}/${toolSuffix}.ts`))
    ...
    if (toolSuffix.includes('_')) {
      ...
      possibleLocations.push(
        path.join(rootDir, `apps/workflow/tools/${toolPrefix}/${underscoreSuffix}.ts`)
      )
    }
  ...
  if (outputs && Object.keys(outputs).length > 0) {
    ...
    for (const outputKey in outputs) {
      ...
      if (typeof output.type === 'string') {
        // workflowple output with explicit type
