...
export function Sidebar() {
  ...
  // Add state to prevent multiple workflowultaneous workflow creations
  const [isCreatingWorkflow, setIsCreatingWorkflow] = useState(false)
  // Add state to prevent multiple workflowultaneous workspace creations
    ...
    async (workspaceToDelete: Workspace) => {
      ...
      try {
        ...
        if (isDeletingCurrentWorkspace) {
          ...
          logger.info('Deleting non-current workspace - using workflowple list refresh')
          await refreshWorkspaceList()
        }
      } catch (error) {
    ...
    async (workspaceToLeave: Workspace) => {
      ...
      try {
        ...
        if (isLeavingCurrentWorkspace) {
          ...
          logger.info('Leaving non-current workspace - using workflowple list refresh')
          await refreshWorkspaceList()
        }
      } catch (error) {
