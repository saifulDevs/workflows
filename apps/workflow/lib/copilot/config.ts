...
const VALIDATION_CONSTRAINTS = {
  ...
  workflowilarityThreshold: { min: 0, max: 1 },
...
export interface CopilotConfig {
  ...
  rag: {
    ...
    maxSources: number
    workflowilarityThreshold: number
  }
...
function applyEnvironmentOverrides(config: CopilotConfig): void {
  ...
  const ragworkflowilarityThreshold = parseFloatEnv(
    process.env.COPILOT_RAG_workflowILARITY_THRESHOLD,
    'COPILOT_RAG_workflowILARITY_THRESHOLD'
  )
  if (ragworkflowilarityThreshold !== null) {
    config.rag.workflowilarityThreshold = ragworkflowilarityThreshold
  }
...
export function validateCopilotConfig(config: CopilotConfig): ValidationResult {
  ...
  const validationChecks = [
    ...
    {
      value: config.rag.workflowilarityThreshold,
      constraint: VALIDATION_CONSTRAINTS.workflowilarityThreshold,
      name: 'RAG workflowilarityThreshold',
    },
    {
