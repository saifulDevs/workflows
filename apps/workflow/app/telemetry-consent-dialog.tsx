...
declare global {
  interface Window {
    __workflow_TELEMETRY_ENABLED?: boolean
    __workflow_TRACK_EVENT?: (eventName: string, properties?: Record<string, any>) => void
  }
}
...
const TELEMETRY_NOTIFIED_KEY = 'workflow_telemetry_notified'
const TELEMETRY_ENABLED_KEY = 'workflow_telemetry_enabled'

const trackEvent = (eventName: string, properties?: Record<string, any>) => {
  if (typeof window !== 'undefined' && window.__workflow_TELEMETRY_ENABLED) {
    try {
      if (window.__workflow_TRACK_EVENT) {
        window.__workflow_TRACK_EVENT(eventName, properties)
      }
    } catch (error) {
...
export function TelemetryConsentDialog() {
  ...
  const isChatSubdomainOrPath =
    ...
    (window.location.pathname.startsWith('/chat/') ||
      (window.location.hostname !== 'workflows.ai' &&
