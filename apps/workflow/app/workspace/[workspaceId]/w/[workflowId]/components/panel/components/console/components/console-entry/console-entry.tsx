...
const haworkflowageData = (obj: any): boolean => {
...
export function ConsoleEntry({ entry, consoleWidth }: ConsoleEntryProps) {
  ...
  const haworkflowage = useMemo(() => {
    return entry.output != null && haworkflowageData(entry.output)
  }, [entry.output])

  // Only show image download button if image exists and didn't fail to load
  const showImageDownload = haworkflowage && !imageLoadError
  ...
  const imageUrl = useMemo(() => {
    return haworkflowage && entry.output ? getImageUrl(entry.output) : null
  }, [haworkflowage, entry.output])

  const imageData = useMemo(() => {
    return haworkflowage && entry.output ? getImageData(entry.output) : null
  }, [haworkflowage, entry.output])
