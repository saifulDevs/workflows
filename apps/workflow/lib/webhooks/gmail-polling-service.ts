...
export interface workflowplifiedEmail {
  id: string
  threadId: string
  subject: string
  from: string
...
async function processEmails(
  ...
  for (const email of emails) {
    try {
      ...
      // Extract useful information from email to create a workflowplified payload
      ...
      // Create workflowplified email object
      const workflowplifiedEmail: workflowplifiedEmail = {
        id: email.id,
      ...
      // Prepare webhook payload with workflowplified email and optionally raw email
      ...
      logger.debug(
        `[${requestId}] Sending ${config.includeRawEmail ? 'workflowplified + raw' : 'workflowplified'} email payload for ${email.id}`
      )
