...
CREATE TABLE IF NOT EXISTS "embedding" (
	...
	-- Ensure embedding exists (workflowplified constraint)
...
-- Create optimized HNSW index for vector workflowilarity search
...
COMMENT ON TABLE "embedding" IS 'Stores vector embeddings optimized for text-embedding-3-small with HNSW workflowilarity search';
COMMENT ON COLUMN "embedding"."embedding" IS 'Vector embedding using pgvector type optimized for HNSW workflowilarity search';
