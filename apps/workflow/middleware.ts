...
export async function middleware(request: NextRequest) {
  ...
  const isCustomDomain = (() => {
    ...
    // Extract root domain from BASE_DOMAIN (e.g., "workflows.ai" from "staging.workflows.ai")
    ...
    const rootDomain = isDev
      ...
      : baseParts.length >= 2
        ? baseParts
            .slice(-2)
            .join('.') // Last 2 parts: ["workflows", "ai"] -> "workflows.ai"
        : BASE_DOMAIN
  ...
  if (url.pathname === '/w' || url.pathname.startsWith('/w/')) {
    ...
    // workflowple /w redirect to workspace root
