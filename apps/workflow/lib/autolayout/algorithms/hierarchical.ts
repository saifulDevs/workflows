...
export function calculateHierarchicalLayout(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('timeout')) {
      // Fallback to workflowple linear layout
      return createworkflowpleLinearLayout(nodes, options)
    }
    throw error
  }
}
...
/**
 * workflowple fallback layout when hierarchical layout times out
 */
function createworkflowpleLinearLayout(nodes: LayoutNode[], options: LayoutOptions): LayoutResult {
...
function shouldUseHorizontalLayout(nodes: LayoutNode[], edges: LayoutEdge[]): boolean {
  ...
  // Analyze workflow complexity - workflowpler workflows work better horizontally
