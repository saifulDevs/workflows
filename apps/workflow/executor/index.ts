...
function trackWorkflowTelemetry(eventName: string, data: Record<string, any>) {
  if (typeof window !== 'undefined' && window.__workflow_TRACK_EVENT) {
    ...
    window.__workflow_TRACK_EVENT(eventName, {
      category: 'workflow',
      ...safeData,
    })
  }
      ...
      while (hasMoreLayers && iteration < maxIterations) {
        ...
        if (this.isDebugging) {
          ...
          if (nextLayer.length === 0) {
            ...
            // Process parallel iterations - workflowilar to loops but conceptually for parallel execution
