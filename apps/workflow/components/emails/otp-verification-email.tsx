...
const baseUrl = env.NEXT_PUBLIC_APP_URL || 'https://workflows.ai'

const getSubjectByType = (type: string, chatTitle?: string) => {
  switch (type) {
    case 'sign-in':
      return 'Sign in to workflow Studio'
    case 'email-verification':
      return 'Verify your email for workflow Studio'
    case 'forget-password':
      return 'Reset your workflow Studio password'
    case 'chat-access':
      return `Verification code for ${chatTitle || 'Chat'}`
    default:
      return 'Verification code for workflow Studio'
  }
}
  ...
  const getMessage = () => {
    switch (type) {
      case 'sign-in':
        return 'Sign in to workflow Studio'
      case 'forget-password':
        return 'Reset your password for workflow Studio'
      case 'chat-access':
        return `Access ${chatTitle || 'the chat'}`
      default:
        return 'Welcome to workflow Studio'
    }
  }
          ...
          <Section style={{ padding: '30px 0', textAlign: 'center' }}>
            <Row>
              <Column style={{ textAlign: 'center' }}>
                <Img
                  src={`${baseUrl}/static/workflow.png`}
                  width='114'
                  alt='workflow Studio'
                  style={{
                    margin: '0 auto',
