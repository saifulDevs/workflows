  ...
  async execute(
    ...
    if (!parallelState) {
      ...
      if (parallelType === 'count') {
        ...
        // workflowple parallel - single execution
        parallelCount = 1
        logger.info(`Parallel ${block.id} will execute ${parallelCount} time (workflowple mode)`)
      }
      ...
      // Note: For workflowple count-based parallels without distribution, we don't store items
