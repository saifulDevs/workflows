...
let telemetryConfig = {
  endpoint: env.TELEMETRY_ENDPOINT || 'https://telemetry.workflows.ai/v1/traces',
  serviceName: 'workflow-studio',
  serviceVersion: '0.1.0',
}

if (typeof window !== 'undefined' && (window as any).__workflow_STUDIO_TELEMETRY_CONFIG) {
  telemetryConfig = { ...telemetryConfig, ...(window as any).__workflow_STUDIO_TELEMETRY_CONFIG }
}
...
function initializeClientTelemetry(): void {
  ...
  try {
    const clientSideEnabled =
      (window as any).__workflow_STUDIO_TELEMETRY_CONFIG?.clientSide?.enabled !== false
