  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url, token }, request) => {
      ...
      const result = await resend.emails.send({
        from: `workflow Studio <team@${getEmailDomain()}>`,
        to: user.email,
    ...
    emailOTP({
      sendVerificationOTP: async (data: {
        ...
        try {
          ...
          const result = await resend.emails.send({
            from: `workflow Studio <onboarding@${getEmailDomain()}>`,
            to: data.email,
          ...
          getUserInfo: async (tokens) => {
            try {
              const response = await fetch('https://oauth.reddit.com/api/v1/me', {
                headers: {
                  Authorization: `Bearer ${tokens.accessToken}`,
                  'User-Agent': 'workflow-studio/1.0',
                },
              })
            ...
            sendInvitationEmail: async (data: any) => {
              try {
                ...
                await resend.emails.send({
                  from: `workflow Studio <team@${getEmailDomain()}>`,
                  to: invitation.email,
                  subject: `${inviterName} has invited you to join ${organization.name} on workflow Studio`,
                  html,
                })
              } catch (error) {
